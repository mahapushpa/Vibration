...
#-------------------------------------------------------------------------------
from common.core.common import channel_to_sensor, sensor_to_unit
...
#-------------------------------------------------------------------------------
def raise_key_error_msg(msg, result = False, do_print = True):
    """Raise KeyError in debug mode, log otherwise.
    Args:
        msg (str): Error message.
        do_print (bool): Whether to also print to console.
    Returns:
        False in production mode, raises KeyError in debug mode.
    """
    from common.core.sys_config import get_sys_value
    ...
    if get_sys_value('debug_mode'):
        logging.debug(traceback.format_exc())  # Add full traceback for debugging
...
def normalize_value(value):
    ...
    if isinstance(value, str):
        ...
        if lowered in ['yes', 'true', '1']: return 1
        if lowered in ['no', 'false', '0']: return 0
        if value.strip().isdigit(): return int(value.strip())
        # return value.strip()  # keep string

    if isinstance(value, bool): return int(value)
    if isinstance(value, (int, float, bytes)): return value
...
def validate_param_value(module, param, value_raw):
    ...
    Returns:
        (success: bool, value: normalized or raw if failed)
    """
    ...
    try:
        ...
        if expected_type.startswith("int") or expected_type.startswith("uint"):
            ...
            if "range" in param_info:
                ...
                if not (lo <= value <= hi):
                    return False, raise_key_error_msg(f"{value} out of range [{lo}-{hi}] for {module}:{param}")
            if "allowed" in param_info and value not in param_info["allowed"]:
                return False, raise_key_error_msg(f"{value} not allowed for {module}:{param}")
            ...
            if "pattern" in param_info and not re.fullmatch(param_info["pattern"], value):
                return False, raise_key_error_msg(f"{value} does not match pattern for {module}:{param}")
            if "size" in param_info and len(value) > param_info["size"]:
                return False, raise_key_error_msg(f"{value} too long (max {param_info['size']}) for {module}:{param}")
            if "allowed" in param_info and value not in param_info["allowed"]:
                return False, raise_key_error_msg(f"{value} not allowed for {module}:{param}")

        elif expected_type == "bytes":
            if not isinstance(cleaned, (bytes, bytearray)):
                return False, raise_key_error_msg(f"Expected bytes for {module}:{param}, got {type(cleaned).__name__}")
            if "size" in param_info and len(cleaned) > param_info["size"]:
                return False, raise_key_error_msg(f"{len(cleaned)} bytes too long (max {param_info['size']})")

            value = cleaned  # no conversion

        else:
            return False, raise_key_error_msg(f"Unsupported type '{expected_type}' for {module}:{param}")

    except Exception as e:
        return False, raise_key_error_msg(f"Validation failed for {module}:{param} - {e}")
...
def validate_module_param(key, module, param, description = "Validate Module and Parameter"):
    ...
    if module not in CMD_TABLE or param not in CMD_TABLE[module]:
        return raise_key_error_msg(f"[CMD_TABLE] Invalid module/param: {module}.{param} of {key}")
...
def format_filter_description():
    ...
    """
    from common.core.parameters import FilterParams
