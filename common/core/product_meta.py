...
#            geo_code = self.get_ip_geolocation() or 'Please add in user_details.ini'
            geo_code = self.get_ip_geolocation_dms() or 'Please add in user_details.ini'
            comment = ' (IP approx)' if geo_code != 'Please add in user_details.ini' else ' (Geo missing)'
    ...
    def get_ip_geolocation():
        ...
        try:
            ...
            data = response.json()
            loc = data.get('loc')  # Example format: "26.9124,75.7873"
            if loc:
                lat, lon = loc.split(',')
                # print(f"[INFO] user_details.ini saved (auto generated):")
                # print(f"Latitude  : {lat.strip()} N")
                # print(f"Longitude : {lon.strip()} E")
                
                return f"{lat.strip()} N {lon.strip()} E"
        except Exception:
            return None
    @staticmethod
