...
from vibmshared.utils.sys_helpers import validate_sys_value
from vibmshared.core.common       import (
...
#-------------------------------------------------------------------------------
    ...
    def load(cls, abs_paths: dict, rel_paths: dict):
        ...
        from vibmshared.utils.utils_helpers import log_info_msg
...
#-------------------------------------------------------------------------------
    ...
    def save(cls, meta_action = "last_edit_save_on"):
        ...
        logging.info(f"sys_config.ini saved ({meta_action})")
        return meta_action
...
def set_sys_value(key, value, validate = True, autosave = True):
    ...
    if autosave:
        ...
        logging.info(f"[INFO] sys_config.ini loaded ({meta_action})")
...
def set_sys_value_default(key, value):
    ...
    # Update the default in whichever group already defines this key.
    # (The previous code wrote to a non-existent "Flags" group -> KeyError.)
    for group in SystemConfig.defaults_by_group.values():
        if key in group:
            group[key] = value
            return
    raise KeyError(f"[SystemConfig] Unknown default key: '{key}'")
    
#-------------------------------------------------------------------------------
def sys_reset_to_defaults(meta_action = "restored_defaults"):
    # build_defaults() requires the path settings; reuse the current
    # default_PathSettings (already resolved during load) so a reset keeps
    # the existing log/data/config paths instead of raising TypeError.
    SystemConfig.build_defaults(default_PathSettings)
    flat_defaults = {
        k.strip(): str(v).strip()
...
def sys_config_init(path_mgr):
    from vibmshared.core.common import default_LocalParameters
...
if __name__ == '__main__':
    ...
    # print(f"[INFO] sys_config.ini loaded ({ini_status})")
#-------------------------------------------------------------------------------
