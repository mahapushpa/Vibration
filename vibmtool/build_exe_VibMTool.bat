...
SET LOCAL_MODULES=%PRJ_ROOT%\VibMShared\core %PRJ_ROOT%\VibMShared\modules %PRJ_ROOT%\VibMShared\utils
...
REM ====================================================
REM           REMOVE __pycache__ folder from VibMShared
REM ====================================================
echo [INFO] Cleaning up __pycache__ folders...
for /d /r "%PRJ_ROOT%\VibMShared" %%d in (__pycache__) do (
    if exist "%%d" rd /s /q "%%d"
)
...
python %PRJ_ROOT%\VibMShared\tools\generate_hidden_imports.py --output hidden_imports.txt %LOCAL_MODULES% %PROJECT_FILES%
IF ERRORLEVEL 1 (
...
REM ====================================================
REM           OBFUSCATE 'VibMShared/' USING PYARMOR
REM -O - is capital 'O', small is not acceptable
REM ====================================================
echo [INFO] Obfuscating 'VibMShared/' with PyArmor...
rmdir /s /q %PRJ_ROOT%\VibMShared_obf >nul 2>&1

:: Only obfuscate needed folders
pyarmor --silent gen %PRJ_ROOT%\VibMShared\core -O %PRJ_ROOT%\VibMShared_obf\core
pyarmor --silent gen %PRJ_ROOT%\VibMShared\utils -O %PRJ_ROOT%\VibMShared_obf\utils
pyarmor --silent gen %PRJ_ROOT%\VibMShared\modules -O %PRJ_ROOT%\VibMShared_obf\modules
...
pyinstaller ^
  ...
  --add-data "%PRJ_ROOT%\VibMShared_obf\core;./VibMShared" ^
  --add-data "%PRJ_ROOT%\VibMShared_obf\utils;./VibMShared" ^
  --add-data "%PRJ_ROOT%\VibMShared_obf\modules;./VibMShared" ^
  --add-data "%PRJ_ROOT%\VibMShared\core\maps_logo.ico;./VibMShared/core" ^
...
copy /Y %PRJ_ROOT%\VibMShared\tools\readme_client.txt "%DIST_DIR%\readme.txt" >nul 2>&1
...
IF EXIST "%PRJ_ROOT%\build_temp" rmdir /s /q "%PRJ_ROOT%\build_temp"
IF EXIST "%PRJ_ROOT%\VibMShared_obf" rmdir /s /q "%PRJ_ROOT%\VibMShared_obf"
