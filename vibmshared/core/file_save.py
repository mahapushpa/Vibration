...
from vibmshared.core.product_meta   import UserMeta
from vibmshared.utils.utils_helpers import get_sensor_unit, safe_log
from vibmshared.core.parameters     import PlotParams, DataHdrParams
from vibmshared.core.sys_config     import sys_config_init, get_sys_value, set_sys_value
from vibmshared.core.common         import set_session_flag, channel_to_sensor, dt_sample, dt_sample_hdr
...
#-------------------------------------------------------------------------------
    def write_file_with_metadata(self, filename, date_str, time_str, data, event_type):
        if not self.write_metadata(filename, date_str, time_str, event_type):
            safe_log(None, f"Aborting: metadata write failed for {filename}", tag = "error", do_print = True)
            return False

        if not self.write_data(filename, data):
            safe_log(None, f"Data write failed for {filename} (metadata was written)", tag = "error", do_print = True)
            return False
...
#-------------------------------------------------------------------------------
    def write_data(self, filename, data):
        try:
            ...
            safe_log(None, f"Data write failed: {e}", tag = "error", do_print = True)
...
#-------------------------------------------------------------------------------
    def write_metadata(self, filename, date_str, time_str, event_type = None):
        ...
        try:
            ...
            safe_log(None, f"Metadata write failed: {e}", tag = "error", do_print = True)
            logging.debug(traceback.format_exc())
            return False
...
#-------------------------------------------------------------------------------
    def save_event_data(self, filename, t_ydata, start_time, event_type):
        try:
            ...
            if not self.write_file_with_metadata(filename, date_str, time_str, data, event_type):
                return False

            rel_path = self.path_handler.get_relative_path(filename)
            safe_log(None, f"[Event] Data saved to {rel_path}", do_print = True)
            return True

        except Exception as e:
            safe_log(None, f"Event save failed: {e}", tag = "error", do_print = True)
            logging.debug(traceback.format_exc())
            return False
