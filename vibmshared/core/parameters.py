# ------------------------------------------------------------------------------
from vibmshared.utils.utils_helpers  import get_channel_sensor_unit
...
class FilterParams:
    ...
    def get_selected(cls):
        from vibmshared.core.sys_config import SystemConfig
...
class TFParams:
    ...
    def get_selected_analysis_type(cls):
        from vibmshared.core.sys_config import SystemConfig
        analysis_type = SystemConfig.current.get('analysis_type', 'Unknown')
        if analysis_type == 'Unknown':
            analysis_type = 'DT'
            print(f"[WARN] Unknown analysis_type, using default: {analysis_type}")
...
class PlotParams:
    ...
    def get_event_span(cls):
        """Whole-second [start, end) window to extract around a centred event:
        PRE_EVENT_SECOND before the middle of the display window through
        PRE_EVENT_SECOND after it — symmetric, (2 * PRE_EVENT_SECOND + 1) s wide.
        Default 5 s window with PRE_EVENT_SECOND = 1 -> (1, 4): 1 s pre, the
        event second, 1 s post. (Same result as the old get_event_centre()-based
        form for odd-length windows; for an even window it centres on the
        lower-middle second.)"""
        mid = cls.get_time_x_span() // 2         # middle second, 0-indexed (5 -> 2)
        pre = cls.PRE_EVENT_SECOND
        return (mid - pre, mid + pre + 1)        # [pre before .. event .. pre after]
